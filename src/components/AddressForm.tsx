...
interface LocalityData {
	// object --> fields (types)--API datas
	postalCode: string;
	name: string;
}
type FieldState = 'idle' | 'loading' | 'success' | 'error'; // inputs border color

function AddressForm() {
	const [postalCode, setPostalCode] = useState('');
	const [locality, setLocality] = useState('');
	const [localityData, setLocalityData] = useState<LocalityData[]>([]); // for dropdown
	...
	const handleSelectPlzOption = (code: string) => {
		const selectedLocality =
			localityData.find((item) => item.postalCode === code)?.name ?? '';
		setPostalCode(code);
							...
							return (
								<li
									...
									style={{
										...
										fontSize: '14px',
										backgroundColor:
											index === focusedPostalIndex
												? '#3a3a3a'
												: 'transparent',
										borderBottom: '1px solid #333',
									}}
									onMouseEnter={() =>
										setFocusedPostalIndex(index)
									}
								>
									<span
										style={{
